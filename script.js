...
(function () {
    ...
    function resetarSessaoAtual() {
        ...
        if (session) {
            try {
                ...
                // Se a empresa salva no local não bater com a sessão, força limpeza
                if (data.empresa && data.empresa !== empresaAtual) {
                    ...
                    // Para observadores antigos imediatamente
                    if (typeof FirestoreService !== 'undefined') {
                        FirestoreService.pararObservadores();
                    }
                    // Limpa todo o localStorage vinculado à empresa antiga
    ...
    const DB = {
        ...
        _isSaving: false, // Controle para evitar loops de salvamento
        ...
        // ===== GETALL COM CONTROLE DE DUPLICAÇÃO E FORCE REFRESH =====
        ...
        add: function (collection, item) {
            // Evita salvamento duplicado durante sincronização
            if (this._isSaving) {
            ...
            try {
                ...
                // Sincroniza com Firestore APENAS se não for uma sincronização já em andamento
        ...
        // Método para sincronizar dados do Firestore sem causar loops
        syncFromFirestore: function(collection, firestoreData) {
            ...
            try {
                ...
                // Mescla dados, dando prioridade ao Firestore (mais recente)
        ...
        _mergeData: function(localData, firestoreData) {
            ...
            // Adiciona dados locais primeiro
            localData.forEach(function(item) {
                var id = String(item.id);
                merged[id] = { ...item, _source: 'local' };
            });
            
            // Sobrescreve com dados do Firestore SEMPRE (Firestore é a fonte da verdade)
        ...
        init: function () {
            ...
            if (!hasData) {
                var collections = ['configuracoes', 'clientes', 'servicos', 'agenda', 'equipe',
                    ...
                    'movimentacoes', 'historico_mensal', 'ultimo_reset'];
                var self = this;
    ...
    // ===== RENDER ALL COM CONTROLE DE DUPLICAÇÃO =====
    function renderAll() {
        if (_isRendering) {
    ...
    window.resetarDados = function () {
        if (confirm('ATENÇÃO: Isso irá apagar TODOS os dados do sistema (clientes, serviços, OS, estoque, etc.)!\n\nTem certeza?')) {
            if (confirm('Última confirmação: Esta ação é IRREVERSÍVEL.')) {
                // Para os observadores para evitar sincronização automática
                if (typeof FirestoreService !== 'undefined') {
                    FirestoreService.pararObservadores();
                }
                ...
                // Limpa também os dados do Firestore se possível
    ...
    // ===== FUNÇÕES DE SINCRONIZAÇÃO GLOBAIS =====
    ...
    function renderClientes() {
        ...
        var clientes = DB.getAll('clientes', true); // forceRefresh = true
    ...
    function renderEquipe() {
        // FORÇA A ATUALIZAÇÃO DO CACHE PARA EQUIPE
        DB.forceClearCache('equipe');
        var equipe = DB.getAll('equipe', true); // forceRefresh = true
    ...
    // ==========================================
    // ===== FIM FUNÇÕES DE ASSINATURA DIGITAL =
    // ==========================================
    ...
    function renderMapaIscasFiltrado(pontos) {
        var container = document.getElementById('mapaGrid');
        if (!container) return;

        if (pontos.length === 0) {
            container.innerHTML = '<div style="grid-column:1/-1;text-align:center;color:#999;padding:40px;">Nenhum ponto encontrado</div>';
            return;
        }
        container.innerHTML = pontos.map(function (p) {
    ...
    window.criarNovoPonto = function () {
        ...
        DB.add('pontosIscas', {
            tipo: tipo, nome: nome, clienteId: clienteId, endereco: endereco, posicao: posicao,
            dataInstalacao: dataInstalacao, ultimaManutencao: ultimaManutencao, status: status, observacoes: observacoes
        });
        fecharModal();
        renderAll();
    ...
    window.salvarEdicaoPonto = function (id) {
        ...
        DB.update('pontosIscas', id, { tipo: tipo, nome: nome, clienteId: clienteId, endereco: endereco, posicao: posicao, dataInstalacao: dataInstalacao, ultimaManutencao: ultimaManutencao, status: status, observacoes: observacoes });
        fecharModal();
        renderAll();
    ...
    window.criarNovoCliente = function () {
        ...
        // Renderiza novamente após um pequeno delay para garantir
        setTimeout(function() {
            renderAll();
        }, 100);
    ...
    window.criarNovoServico = function () {
        ...
        // 2. CRIA O AGENDAMENTO MANUALMENTE (EVITA DUPLICAÇÃO)
        ...
        // 3. CRIA A OS VINCULADA
        sincronizarServicoComOS(servico, 'add');
        
        // 4. LIMPA CACHES E RENDERIZA
        DB.forceClearCache('agenda');
        DB._clearAllCaches();
    ...
    window.editarAgenda = function (id) {
        var a = DB.getById('agenda', id);
        if (!a) {
            alert('Agendamento não encontrado!');
            return;
        }

        // Se for um agendamento vinculado a um serviço, redireciona para editar o serviço
        if (a.servicoId) {
            var servico = DB.getById('servicos', a.servicoId);
            if (servico) {
                if (confirm('Este agendamento está vinculado ao Serviço #' + String(servico.id).padStart(3, '0') + 
                           '. Deseja editar o serviço em vez do agendamento?')) {
                    fecharModalAgenda();
        ...
        var clientes = DB.getAll('clientes');
        var clienteNome = a.clienteNome || getClienteNome(a.clienteId);
        
        abrirModal('Editar Agendamento', `
            <div class="form-group">
                <label>Cliente</label>
                <select id="modalAgendaCliente">
                    <option value="">Selecione um cliente</option>
                    ${clientes.map(c => 
                        `<option value="${c.id}" ${c.id === a.clienteId ? 'selected' : ''}>${c.nome} (${c.documento || 'N/A'})</option>`
                    ).join('')}
                </select>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label>Data</label>
                    <input type="date" id="modalAgendaData" value="${a.data ? a.data.split('/').reverse().join('-') : ''}" />
                </div>
                <div class="form-group">
                    <label>Horário</label>
                    <input type="time" id="modalAgendaHorario" value="${a.horario || '09:00'}" />
                </div>
            </div>
            <div class="form-group">
                <label>Descrição</label>
                <textarea id="modalAgendaDescricao" rows="3">${a.descricao || ''}</textarea>
            </div>
            ${a.servicoId ? `
                <div style="background:#f0f7fc;padding:8px 14px;border-radius:8px;margin-bottom:12px;border-left:3px solid #1d7a6b;">
                    <small>🔗 Vinculado ao Serviço #${String(a.servicoId).padStart(3, '0')}</small>
                </div>
            ` : ''}
            <div class="modal-footer">
                <button class="btn-secondary" onclick="fecharModal()">Cancelar</button>
                <button class="btn-primary" onclick="salvarEdicaoAgenda(${id})">Salvar</button>
            </div>
        `);
    };

    window.salvarEdicaoAgenda = function (id) {
        var clienteId = parseInt(document.getElementById('modalAgendaCliente')?.value || '0');
        var data = document.getElementById('modalAgendaData')?.value?.split('-').reverse().join('/') || '';
        var horario = document.getElementById('modalAgendaHorario')?.value || '09:00';
        var descricao = document.getElementById('modalAgendaDescricao')?.value || '';
    ...
    window.excluirAgenda = function (id) {
        ...
        if (a.servicoId) {
            if (!confirm('Este agendamento está vinculado ao Serviço #' + String(a.servicoId).padStart(3, '0') + 
                        '. Deseja excluir o agendamento e desvincular do serviço?')) {
                return;
            }
        } else {
    ...
    window.criarNovoMembro = function () {
        ...
        DB.forceClearCache('equipe'); // Limpa o cache imediatamente
        fecharModal();
    ...
    // Inicia os observadores em tempo real se já estiver logado
